...
| npm run connectivity | Provides a connectivity check to the Tufin_securetrack system.|
| npm run healthcheck | Checks whether the configured healthcheck call works to Tufin_securetrack.|
| npm run basicget | Checks whether the cbasic get calls works to Tufin_securetrack.|
...
The `healthCheck` call ensures that the adapter can communicate with Tufin_securetrack. The actual call that is used is defined in the adapter properties.
```js
